...
However, **that is not all!** As the "vanilla" implementation was found to be somewhat disappointing (for example,  
the `terrainSphereModification` function throwing an error instead of allowing terrain to be placed back down!), the  
mod *also* **immediately and massively expands the system** in various ways by implementing a large custom Lua scripting API.  
